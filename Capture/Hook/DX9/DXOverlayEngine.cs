...
namespace Capture.Hook.DX9
{
    internal class DXOverlayEngine : Component
    {
        ...
        public void SpriteOnLostDevice()
        {
        ...
        public bool Initialise(Device device)
        {
            ...
            try
            {
                ...
                _device = device;
                _nativePtr = device.NativePointer;
        ...
        public void Draw()
        {
            ...
            foreach (var overlay in Overlays)
            {
                foreach (var element in overlay.Elements)
                {
                    ...
                    if (textElement != null)
                    {  //ToDispose is essential! otherwise they are not released and game hangs when released.
                        Font font =ToDispose(GetFontForTextElement(textElement));
                    ...
                    {
                        Texture image = ToDispose(GetImageForImageElement(imageElement));
                        if (image != null)
