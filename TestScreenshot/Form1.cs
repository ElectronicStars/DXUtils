    ...
    public partial class Form1 : Form
    {
        ...
        private void AttachProcess()
        {
            ...
            foreach (Process process in processes)
            {
                ...
                captureInterface.ScreenshotReceived += new ScreenshotReceivedEvent(CaptureInterface_RemoteScreenshotReceived);
        ...
        /// <summary>
        /// Display Screenshot sent from the target process
        /// </summary>
        /// <param name="message"></param>
        void CaptureInterface_RemoteScreenshotReceived(ScreenshotReceivedEventArgs screenshotArgs)
        {
            var screenshot = screenshotArgs.Screenshot;
            ...
            progressBar1.Invoke(new MethodInvoker(delegate()
                {
                    if (progressBar1.Value < progressBar1.Maximum)
                    {
                        ...
                        //test without bringing process forward
                       // _captureProcess.BringProcessWindowToFront();
                        ...
                        _captureProcess.CaptureInterface.BeginGetScreenshot(new Rectangle(int.Parse(txtCaptureX.Text), int.Parse(txtCaptureY.Text), int.Parse(txtCaptureWidth.Text), int.Parse(txtCaptureHeight.Text)), new TimeSpan(0, 0, 10), Callback, resize, (ImageFormat)Enum.Parse(typeof(ImageFormat), cmbFormat.Text));
                    }
                    else
                    {
                        end = DateTime.Now;
